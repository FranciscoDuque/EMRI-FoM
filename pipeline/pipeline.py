# python pipeline.py --M 1e6 --mu 5e1 --a 0.9 --e_f 0.0 --T 4.0 --z 0.1 --repo test --psd_file TDI2_AE_psd.npy --dt 10.0 --tdi2 --channels AET --use_gpu --N_montecarlo 1 --device 0 --repo test
import os
import logging
import argparse
import numpy as np
...
if __name__ == "__main__":
    ...
    # Convert trajectory data to GPU arrays (if using CUDA)
    t, p, e, x, Phi_phi, Phi_theta, Phi_r = traj(M, mu, a, p0, e0, 1.0, T = T, dt = 1e-13)
    ...
    theta, phi = qS, phiS  # Use sky localization angles (qS, phiS)
