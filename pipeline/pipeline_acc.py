...
def parse_arguments():
    ...
    parser.add_argument('--no_tdi', action='store_true', default=False, help="Do not use TDI channels. Use averaged PSD")
...
inspiral_kwargs_back = {"err": 1e-13, "integrate_backwards": True, "func": choose_trajectory()}
inspiral_kwargs_forward = {"err": 1e-13, "integrate_backwards": False, "func": choose_trajectory()}
...
param_names_fisher = np.delete(param_names, popinds).tolist()
...
if __name__ == "__main__":
    ...
    if args.power_law:
        detector_frame_data["A power-law amplitude"] = A
        detector_frame_data["nr power-law index"] = nr
        source_frame_data["A power-law amplitude"] = A
    ...
    traj = EMRIInspiral(func=choose_trajectory())
    ...
    for j in range(args.N_montecarlo):
        ...
        # define the initial parameters
        parameters = np.asarray([M, mu, a, p0, e0, x0, dist, qS, phiS, qK, phiK, Phi_phi0, Phi_theta0, Phi_r0])
        vacuum_parameters = parameters.copy()
        if args.power_law:
            extra_parameters = np.asarray([A, nr])
            parameters = np.append(parameters, extra_parameters)
        else 
        ...
        fish = StableEMRIFisher(*vacuum_parameters, 
                                dt=args.dt, T=T, extra_param_args = extra_parameters, EMRI_waveform_gen=EMRI_waveform_gen, noise_model=psd_wrap, noise_kwargs=dict(TDI="TDI2"), param_names=param_names_fisher, stats_for_nerds=False, use_gpu=args.use_gpu, 
                                der_order=8., Ndelta=24, filename=current_folder,
                                deltas = deltas,
        ...
        # check dimensions
        print("Fisher matrix shape", fim.shape[0]==len(param_names_fisher))
        if log_e:
